...
class TheatreScheduler:
    ...
    def solve(self, solver_name, options=None, solver_path=None, local=True):
        ...
        self.model.SESSION_ASSIGNED[(1, 1001)].pprint()
        self.df_times = pd.DataFrame(results)
        # self.df_times.to_csv('df.csv')

        all_cases = self.model.CASES.value_list
        cases_assigned = []
        for (case, session) in self.model.SESSION_ASSIGNED:
            if pyo.value(self.model.SESSION_ASSIGNED[(case, session)]):
...
if __name__ == "__main__":
    ...
    cbc_path = "D:\\Coding\\Hackathons\\SPITHackathon2023\\spitenv\\Lib\\site-packages\\Cbc-1.1.0-win32-msvc7\\bin\\cbc.exe"
