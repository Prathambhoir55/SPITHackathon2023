...
def summarizerTool(pdf_path):
    ...
    text2 = ''
    for filename in os.listdir('Generate_MCQ_BERT_Wordnet_Conceptnet\\egypt.pdf_dir'):
        f = os.path.join('Generate_MCQ_BERT_Wordnet_Conceptnet\\egypt.pdf_dir', filename)
        print(f)
        img = cv2.imread(f)
...
def api(pdf_path): #main function
...
all_answers, all_questions, all_choices = api('Generate_MCQ_BERT_Wordnet_Conceptnet\\bug\\egypt.pdf')
