...
const Organizations = () => {
  console.log(data);
  const front = ["Physics", "Chemistry", "Math", "Biology", "English"];
  const back = ["Django", "Node", "DWF"];
  // const dataA = [
  // 	"pandas",
  // 	"power BI",
  // 	"Numpy",
    ...
    <div>
      <Tabs
        ...
        <Suspense fallback={<LoadingSpinner />}>
          <TabPanel>
            {!isPending && <AllOrganizations temp={front} id="1" />}
          </TabPanel>
          <TabPanel>
            {!isPending && <AllOrganizations temp={back} id="2" />}
          </TabPanel>
          {/* <TabPanel>
            {!isPending && <AllOrganizations temp={dataA} id="3" />}
          </TabPanel>
          <TabPanel>
            {!isPending && <AllOrganizations temp={Blockchain} id="4" />}
          </TabPanel> */}
          {/* <TabPanel>
            {!isPending && <AllOrganizations temp={front} id="1" />}
          </TabPanel> */}
